...
if($_SERVER["REQUEST_METHOD"] == "POST"){
    ...
    if (empty($userName)){
        ...
        $userName_error="username must not be over 10 characters long";
    }
    //do error checks for username
    if (empty($password)){
        ...
        $password_error="password must not be over 10 characters long";
    }
    //do error check for password check
    if ($password != $passwordCheck){
        $passwordCheck_error="passwords do not match";
    }
